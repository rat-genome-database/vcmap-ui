<template>
  ...
  <template v-for="(blockSection, index) in level1Blocks" :key="index">
    <g @contextmenu.prevent="showContextMenu({ event: $event, region, section: blockSection })">
      ...
      <rect class="block-section" @mouseenter="onMouseEnter($event, blockSection, 'trackSection')"
        ...
        @click="onSyntenyBlockClick(blockSection, $event)" :y="blockSection.posY1" :x="blockSection.posX1"
  ...
  <template v-for="(blockSection, index) in level2Blocks" :key="index">
    ...
    <rect class="level-2 block-section" @mouseenter="onMouseEnter($event, blockSection, 'trackSection')"
      @mouseleave="onMouseLeave(blockSection)" @mousemove="updatePositionLabelFromMouseEvent($event, blockSection)"
      :y="blockSection.posY1" :x="blockSection.posX1" :width="blockSection.width" :height="blockSection.height"
      :fill="getSectionFill(blockSection)" :fill-opacity="1" />
    <ChromosomeLabelSVG v-if="showChromosome" :synteny-section="blockSection" />
  </template>
