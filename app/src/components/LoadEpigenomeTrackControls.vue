<template>
    <Dialog 
      ...
      <div class="content-container">
        <Button label="Load Epigenome Data Track" icon="pi pi-plus" class="p-button-secondary p-button-sm add-track-btn" @click="onAddEpigenomeDataTrack" />
      ...
      <div class="grid p-d-flex modal-controls-container">
          ...
          <div>
            <Button label="Load Epigenome Data Track" class="p-button-secondary" icon="pi pi-arrow-right" @click="onConfirmLoadEpigenomeDataTrack" />
  ...
  interface Props 
  {
    ...
    onCloseLoadEpigenomeDataTrackModal: () => void;
  ...
  interface EpigenomeDataTrackItem
  {
    species: SpeciesMap;
  }
  ...
  // refs
  const dataTrackItems = ref<EpigenomeDataTrackItem[]>([]);
  ...
  const onConfirmLoadEpigenomeDataTrack = () => {
    const selectedMapKeys: number[] = [];
    dataTrackItems.value.forEach((item) => {
      if (item.species) {
        selectedMapKeys.push(item.species.key);
